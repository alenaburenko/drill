...
const translations = {
  uk: {
    ...
    importJsonDesc: 'Ви можете імпортувати власні задачі або цілу базу. Формат має відповідати інтерфейсу DrillTask [].',
    ...
    exportImportDesc: 'Ви можете скопіювати резервну копію вашого поточного прогресу, або вставити раніше збережену копію для відновлення.',
    ...
    // Stage headers
    stageHeaders: [
      { title: 'Етап 1: Вивчення коду', desc: 'Уважно вивчіть умову задачі, готове еталонне рішення та докладний розбір логіки нижче. Коли будете готові перейти до практики — натисніть кнопку внизу.' },
      { title: 'Етап 2: Переписування рішень', desc: 'Зліва відображено готове еталонне рішення. Ваше завдання — повністю переписати його руками в редакторі справа і запустити тести. Жодного копіювання!' },
      { title: 'Етап 3: Відтворення з підказкою', desc: 'Рішення повністю приховано. Спробуйте написати код самостійно. Якщо виникнуть труднощі, ви можете скористатися кнопкою «Підглянути».' },
      { title: 'Етап 4: Заповнення пропусків', desc: 'Нижче наведено код з пропущеними ключовими рядками. Відновіть відсутні частини логіки замість коментарів /* ??? */.' },
      { title: 'Етап 5: З чистого листа', desc: 'Орієнтуйтеся тільки на опис задачі та початкову заготовку функції. Рішення повністю приховано.' },
      { title: 'Етап 6: Live-coding Екзамен', desc: 'Найсуворіший етап. Чистий редактор, увімкнений таймер, переглянути рішення неможливо. Пройдіть тести!' },
      { title: 'Етап 7: Задача освоєна! 🎉', desc: 'Вітаємо! Ви повністю вивчили та заучили рішення цієї задачі за інтервальним методом.' },
    ],
    ...
    allTestsPassed: (next: boolean) => `🎉 ВІТАЄМО! Всі тести пройдено успішно. ${next ? 'Переходимо на наступний етап...' : 'Екзамен здано!'}`,
    testPassed: 'Passed',
    testFailed: 'Failed',
    testError: 'Помилка:',
    category: 'Категорія:',
  ...
  en: {
    ...
    importJsonDesc: 'You can import your own tasks or an entire database. Format must match the DrillTask [] interface.',
    ...
    exportImportDesc: 'You can copy a backup of your current progress, or paste a previously saved backup to restore it.',
    ...
    restoreWarning: 'Warning: Importing progress will overwrite your current stats and session history in localStorage.',
    ...
    stageHeaders: [
      { title: 'Stage 1: Study the Code', desc: 'Carefully study the task description, the reference solution, and the detailed breakdown below. When ready to start practicing — click the button below.' },
      { title: 'Stage 2: Retype the Solution', desc: 'The reference solution is shown on the left. Your task is to fully retype it by hand in the editor on the right and run the tests. No copy-paste!' },
      { title: 'Stage 3: Reproduce from Memory', desc: 'The solution is fully hidden. Try to write the code on your own. If you struggle, you can use the "Peek" button (each peek is recorded).' },
      { title: 'Stage 4: Fill in the Blanks', desc: 'Below is code with missing key lines. Restore the missing logic instead of the /* ??? */ comments to pass the tests.' },
      { title: 'Stage 5: From Scratch', desc: 'Only the task description and the starter function skeleton are available. The solution is fully hidden, and hints are disabled at this stage.' },
      { title: 'Stage 6: Live-coding Exam', desc: 'The hardest stage. Clean editor, active timer, no peeking. Pass the tests to prove full mastery!' },
      { title: 'Stage 7: Task Mastered! 🎉', desc: 'Congratulations! You have fully learned and memorized this task using the spaced repetition method.' },
    ],
    ...
    examActiveDesc: 'Hints are fully disabled. Time is limited. You must independently reproduce the code and pass the tests.',
    ...
    allTestsPassed: (next: boolean) => `🎉 GREAT JOB! All tests passed. ${next ? 'Moving to next stage...' : 'Exam passed!'}`,
    testPassed: 'Passed',
    testFailed: 'Failed',
    testError: 'Error:',
    category: 'Category:',
