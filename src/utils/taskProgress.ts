...
export function isTaskDue(
  ...
  const dueTime =
    new Date(prog.lastPracticed).getTime() +
    (STAGE_INTERVALS_HOURS[prog.learningStage] || 24) * msInHour;
...
export function getNewTasks(
  ...
  return allTasks.filter(
    t => !progressMap[t.id] || progressMap[t.id].learningStage === 1,
  );
}

/** Get tasks that are due for repetition */
export function getDueRepetitions(
  allTasks: DrillTask[],
  progressMap: Record<string, UserProgress>,
): DrillTask[] {
...
export function getMasteredTasks(
  allTasks: DrillTask[],
  progressMap: Record<string, UserProgress>,
): DrillTask[] {
...
export function getInProgressTasks(
  ...
  return allTasks.filter(
    t =>
      progressMap[t.id] &&
      progressMap[t.id].learningStage > 1 &&
      progressMap[t.id].learningStage < 7,
  );
}
...
export function getStats(
  ...
  return {
    ...
    completionRate:
      allTasks.length > 0
        ? Math.round((masteredTasks.length / allTasks.length) * 100)
        : 0,
  };
}
...
export function pickNextTask(
  ...
  const activePracticed = inProgressTasks
    ...
    .sort(
      (a, b) =>
        new Date(progressMap[b.id].lastPracticed!).getTime() -
        new Date(progressMap[a.id].lastPracticed!).getTime(),
    );
