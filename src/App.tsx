  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden">
      {drops.map(d => (
        ...
        >
          <span className="text-[8px] font-mono" style={{ color: 'var(--neon-cyan)' }}>
            {d.char}
          </span>
        </div>
      ))}
...
export default function App() {
  ...
  const handleCreateCustomTask = (e: React.FormEvent) => {
    ...
    setNewTitle('');
    setNewDescription('');
    setNewStarter(defaults.starter);
    setNewSolution(defaults.solution);
    setNewCloze1(defaults.clozeSteps[0]);
    setNewCloze2(defaults.clozeSteps[1]);
    setNewCloze3(defaults.clozeSteps[2]);
    setNewBreakdown(defaults.breakdown);
    setNewTestCode(defaults.testCode);
      ...
      for (const item of arr) {
        if (item.title && item.starter && item.solution && item.testCode) {
          validTasks.push({
            ...
            clozeSteps: item.clozeSteps || [
              item.cloze || item.starter,
              item.cloze || item.starter,
              item.cloze || item.starter,
            ],
  ...
  if (bootPhase === 'flash' || bootPhase === 'boot') {
    return (
      <div
        className="min-h-screen flex flex-col items-center justify-center crt-curve"
        style={{ background: 'var(--bg-base)', color: 'var(--neon-cyan)' }}
      >
        ...
        <div className="font-mono text-center">
          <div className="text-sm mb-8" style={{ color: 'var(--neon-green)' }}>
            ...
            <div className="boot-line" style={{ animationDelay: '0.3s' }}>
              MEMORY CHECK: OK
            </div>
            <div className="boot-line" style={{ animationDelay: '0.6s' }}>
              CRT DISPLAY: 60Hz
            </div>
            <div className="boot-line" style={{ animationDelay: '0.9s' }}>
              LOADING DRILL MODULE...
            </div>
  ...
  return (
    <div
      className="min-h-screen flex flex-col crt-curve crt-warp-container overflow-x-hidden"
          ...
          >
            <div className="workspace-glitch-in">
              <TaskView
                ...
                onBack={() => {
                  playClick();
                  setRoute({ tab: activeTab === 'backup' ? 'backup' : 'catalog', taskId: null });
                }}
                lang={lang}
              />
            </div>
          ...
          >
            ...
            <div className="flex-1 max-w-7xl w-full mx-auto p-6 flex flex-col gap-6">
              {activeTab === 'dashboard' && (
                <Suspense
                  fallback={
                    <div className="text-center py-20 font-mono text-[var(--text-muted)]">Loading dashboard...</div>
                  }
                >
                  <DashboardView
                    allTasks={allTasks}
              ...
              {activeTab === 'catalog' && (
                <CatalogView allTasks={allTasks} progressMap={progressMap} lang={lang} catalog={catalog} />
              )}
              {activeTab === 'upload' && (
                <Suspense
                  fallback={<div className="text-center py-20 font-mono text-[var(--text-muted)]">Loading...</div>}
                >
                  <UploadPanel
                    t={t}
              ...
              <div
                ...
                style={{ animation: 'scroll-indicate 2.5s ease-in-out infinite' }}
              >
