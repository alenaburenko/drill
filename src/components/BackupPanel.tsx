...
export default function BackupPanel({ t, progressBackupString, onGenerateBackup, onRestore }: Props) {
  return (
    <Card padding="lg">
      <SectionHeader icon={<Database className="w-4 h-4" />} title={t.exportImport} subtitle={t.exportImportDesc} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          ...
          <p className="text-xs font-mono" style={{ color: 'var(--neon-amber)' }}>
            {t.restoreWarning}
          </p>
          <textarea
            id="progress-import-textarea"
            rows={6}
          ...
          <Button
            onClick={() => {
              ...
              if (val) {
                if (window.confirm(t.restoreConfirm)) onRestore(val);
