...
const StageMastered: React.FC<Props> = ({ t, peeksCount }) => (
  ...
  >
    ...
    <div
      className="px-6 py-4 rounded-xl text-xs text-left w-full max-w-xs font-mono space-y-2 border"
      style={{ background: 'rgba(0,0,0,0.4)', borderColor: 'var(--border)' }}
    >
      <div className="font-bold mb-1 uppercase tracking-wider text-[10px]" style={{ color: 'var(--accent)' }}>
        {t.masteryResults}
      </div>
      {[
        ...
        <div key={k as string} className="flex justify-between">
          ...
          <span className="font-bold" style={{ color: c as string }}>
            {v}
          </span>
        </div>
      ))}
