              ...
              {dueRepetitions.slice(0, 4).map(task => {
                ...
                return (
                  <Card key={task.id} variant="elevated" padding="md" onClick={() => { playTaskClick(); onSelectTask(task.id); }}
                    ...
                    <h4 className="font-bold truncate transition-colors group-hover:text-[var(--neon-cyan)]" style={{ fontSize: '15px' }}>{task.title}</h4>
                    <div className="font-bold flex items-center gap-1 mt-2" style={{ color: 'var(--neon-cyan)', fontSize: '13px' }}>
              ...
              <Card key={task.id} variant="elevated" padding="sm" onClick={() => { playTaskClick(); onSelectTask(task.id); }}
                ...
                <div className="flex items-center gap-3 truncate">
                  ...
                  <div className="truncate">
                    <h4 className="font-bold truncate transition-colors group-hover:text-[var(--neon-cyan)]" style={{ fontSize: '15px' }}>{task.title}</h4>
                    <span className="font-mono" style={{ color: 'var(--text-muted)', fontSize: '13px' }}>{task.block}</span>
                  </div>
                </div>
